...
use crate::open_assets::{OpenAsset, OpenAssetOutput, compute_assets};
...
impl Query {
    ...
    pub fn load_assets(&self, tx: &Transaction) -> Result<Vec<Option<OpenAsset>>> {
        let network = tapyrus::network::constants::Network::from(self.config.network_type);
    ...
    pub fn open_assets(&self, utxos: &Vec<Utxo>) -> Result<HashMap<OutPoint, OpenAsset>> {
        ...
        utxos.iter().for_each(|o|
            match self.lookup_txn(&o.txid) {
                Some(tx) => {
